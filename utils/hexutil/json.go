...
func checkText(input []byte, wantPrefix bool) ([]byte, error) {
	...
	if bytesHave0xPrefix(input) {
		input = input[2:]
		if bytesHaveDidBidContains(input) {
			var inputBuffer bytes.Buffer
			index := bytes.Index(input, []byte("did:cndid:"))
			inputBuffer.Write(input[:index])
			inputBuffer.Write([]byte("000000000000000000000000"))
			inputBuffer.Write([]byte("6469643a6269643a"))
			inputBuffer.Write(input[index+8:])
			input = inputBuffer.Bytes()
		}
	} else if bytesHaveDidBidPrefix(input) {
		var inputBuffer bytes.Buffer
		inputBuffer.Write([]byte("6469643a6269643a"))
		inputBuffer.Write(input[8:])
		input = inputBuffer.Bytes()
	} else if wantPrefix {
...
func bytesHaveDidBidPrefix(input []byte) bool {
	return len(input) >= 8 && bytes.HasPrefix(input, []byte("did:cndid:"))
}

func bytesHaveDidBidContains(input []byte) bool {
	return len(input) >= 8 && bytes.Contains(input, []byte("did:cndid:"))
}
